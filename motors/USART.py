...
def set_pid(datap,datai,datad):
    TYPEP = datap
    TYPEI = datai
    TYPED = datad
    send_data("$mpid:{},{},{}#".format(TYPEP,TYPEI,TYPED))
...
def set_motor_parameter():
    ...
    if MOTOR_TYPE == 1:
        ...
        time.sleep(0.1)
        set_wheel_dis(94.00)  # 配置轮子直径，测量得出  Configure the wheel diameter and measure it
        time.sleep(0.1)
        set_motor_deadzone(2200)  # 配置电机死区，实验得出  Configure the motor dead zone, and the experiment shows
        time.sleep(0.1)
...
if __name__ == "__main__":
    try:
        ...
        time.sleep(0.1)
        send_data("$read_flash#")
        received_message = receive_data()
        if received_message: print(received_message)
